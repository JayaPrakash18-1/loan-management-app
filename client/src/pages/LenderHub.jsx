...
export default function LenderHub({ token }) {
  ...
  const fetchLoans = () => {
    fetch('http://localhost:3000/loans/given', {
  ...
  const handleApprove = async (e) => {
    e.preventDefault();
    try {
      await fetch(`http://localhost:3000/payments/${showVerifyModal.id}/approve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleManualPayment = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`http://localhost:3000/payments/${showManualModal.id}/manual`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleReject = async (paymentId) => {
    ...
    try {
      await fetch(`http://localhost:3000/payments/${paymentId}/reject`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }
      });
      fetchPending();
